...
public class WaitSleepTest {
    ...
    private static class WaitThread implements Runnable{
        ...
        public void run() {
            synchronized (object){
                System.out.println("com.jfs.thread:"+Thread.currentThread().getName()+"enter,Time:"+System.currentTimeMillis());
                try {
                    object.wait();//Object 方法，线程释放当前对象锁进入等待对象锁定池进行等待
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
                System.out.println("com.jfs.thread:"+Thread.currentThread().getName()+"exit,Time:"+System.currentTimeMillis());
            }
        }
    }
